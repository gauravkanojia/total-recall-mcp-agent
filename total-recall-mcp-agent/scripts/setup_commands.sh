...
(
  ...
  sleep 0.5
  echo '{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"remember_memory","arguments":{"content":"User prefers dark mode","kind":"preference"}}}'
  sleep 0.5
  echo '{"jsonrpc":"2.0","id":4,"method":"tools/call","params":{"name":"recall_memory","arguments":{"query":"theme preference","kind":"preference"}}}'
  sleep 1
) | uv run total-recall-mcp-agent

# Verify data in CockroachDB is running
...
# Confirm audit logging data is being recorded

# You should see STARTED rows for both tools.
podman exec roach-single ./cockroach sql --insecure \
  --host=localhost:26257 --database=total_recall_mcp_db \
  -e "
SELECT tool_name, status, created_at
FROM audit_logs
WHERE tool_name IN ('remember_memory', 'recall_memory')
ORDER BY created_at DESC
LIMIT 10;
"

# Then in chat, ask something like:
Use remember_memory to store: "User prefers dark mode" as kind preference.
Then use recall_memory with query "theme preference".